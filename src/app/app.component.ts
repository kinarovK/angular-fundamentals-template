...
export class AppComponent implements OnInit, OnDestroy {
  ...
  initCharacterEvents(): void {
    ...
    this.charactersResults$ = this.searchTermByCharacters.pipe(
      ...
      switchMap((seachterm: string) =>
        this.mockDataService.getCharacters(seachterm)
      )
    );
    // YOUR CODE ENDS HERE
  }
  ...
  loadCharactersAndPlanet(): void {
    ...
    this.planetAndCharactersResults$ = forkJoin([
      ...
      map(([characters, planets]) => {
        ...
        return [
          ...characterNames.map((name: any) => ({ name })),
          ...planetNames.map((name: any) => ({ name })),
        ];
      })
    );
    console.log("hmmm");
  ...
  initLoadingState(): void {
    ...
    combineLatest([charactersLoader, planetsLoader]).subscribe(
      (loaderStatus: boolean[]) => {
        this.isLoading = this.areAllValuesTrue(loaderStatus);
      }
    );
    // YOUR CODE ENDS HERE
  }
